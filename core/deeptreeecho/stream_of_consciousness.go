...
// GetAllThoughts returns a defensive snapshot of all retained thoughts.
func (soc *StreamOfConsciousness) GetAllThoughts() []AutonomousThought {
	soc.mu.RLock()
	defer soc.mu.RUnlock()

	thoughts := make([]AutonomousThought, len(soc.thoughts))
	copy(thoughts, soc.thoughts)
	for i := range thoughts {
		thoughts[i].Tags = append([]string(nil), thoughts[i].Tags...)
	}
	return thoughts
}
