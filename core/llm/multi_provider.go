...
func (mp *MultiProviderLLM) initializeProviders() {
	...
	// Always retain a deterministic local fallback as the final provider. Remote
	// outages must degrade cognitive richness, not terminate autonomous thought.
	remoteProviders := len(mp.providers)
	mp.AddProvider(&SimpleFallbackProvider{})
	if remoteProviders == 0 {
